...
if (isset($_POST['save_brand'])) {
    ...
    if (empty($name)) {
        $message = '<div class="alert alert-danger">Brand name is required!</div>';
    } else {
        if ($brand_id > 0) {
            $conn->query("UPDATE brands SET name='$name', description='$description' WHERE id=$brand_id");
            $message = '<div class="alert alert-success">Brand updated successfully!</div>';
        } else {
            $conn->query("INSERT INTO brands (name, description) VALUES ('$name', '$description')");
