...
if (isset($_POST['update_status'])) {
    ...
    // Only allow status change if assigned to this admin/staff
    $order = $conn->query("SELECT assigned_to FROM orders WHERE id=$order_id")->fetch_assoc();
    if ($order && ($order['assigned_to'] == $admin_id)) {
        $conn->query("UPDATE orders SET status='$status' WHERE id=$order_id");
        $_SESSION['message'] = "Order status updated!";
    } else {
        $_SESSION['message'] = "You are not assigned to this order.";
    }
}
...
if (isset($_POST['assign_order'])) {
    ...
    if ($order && empty($order['assigned_to'])) {
        $conn->query("UPDATE orders SET assigned_to=$admin_id WHERE id=$order_id");
        $_SESSION['message'] = "Order assigned to you.";
    } else {
        $_SESSION['message'] = "Order is already assigned.";
    }
}
...
// Get orders (exclude user-cancelled orders)
if (!empty($status_filter)) {
    $orders = $conn->query("SELECT * FROM orders WHERE status='$status_filter' AND status != 'cancelled_by_user' ORDER BY created_at DESC")->fetch_all(MYSQLI_ASSOC);
} else {
    $orders = $conn->query("SELECT * FROM orders WHERE status != 'cancelled_by_user' ORDER BY created_at DESC")->fetch_all(MYSQLI_ASSOC);
}
...
if (isset($_GET['view'])) {
    ...
    $order_result = $conn->query("SELECT * FROM orders WHERE id=$order_id AND status != 'cancelled_by_user'");
    if ($order_result->num_rows > 0) {
        $view_order = $order_result->fetch_assoc();
        // Get assigned staff/admin info
        $assigned_admin = null;
        if (!empty($view_order['assigned_to'])) {
            $admin_result = $conn->query("SELECT username, email FROM admins WHERE id=" . intval($view_order['assigned_to']));
            if ($admin_result && $admin_result->num_rows > 0) {
                $assigned_admin = $admin_result->fetch_assoc();
            }
        }
        // Get order items
        ...
        $view_order['assigned_admin'] = $assigned_admin;
    }
}
?>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin: 20px 0;">
                        ...
                        <div>
                            ...
                            <p><strong>Assigned To:</strong> 
                                <?php if ($view_order['assigned_admin']): ?>
                                    <?php echo htmlspecialchars($view_order['assigned_admin']['username']); ?> (<?php echo htmlspecialchars($view_order['assigned_admin']['email']); ?>)
                                <?php else: ?>
                                    <span style="color: #888;">Unassigned</span>
                                <?php endif; ?>
                            </p>
                            <p><strong>Status:</strong>
                                <?php if (empty($view_order['assigned_to'])): ?>
                                    <form method="POST" style="display:inline;">
                                        <input type="hidden" name="order_id" value="<?php echo $view_order['id']; ?>">
                                        <button type="submit" name="assign_order" class="btn btn-primary" style="padding: 5px 12px; font-size: 13px;">Assign to Me</button>
                                    </form>
                                <?php elseif ($view_order['assigned_to'] == $_SESSION['admin_id']): ?>
                                    <form method="POST" style="display: inline;">
                                        <input type="hidden" name="order_id" value="<?php echo $view_order['id']; ?>">
                                        <select name="status" onchange="this.form.submit()" style="padding: 5px; border-radius: 3px;">
                                            ...
                                            <!-- Cancelled status removed for staff -->
                                        </select>
                                        <input type="hidden" name="update_status" value="1">
                                    </form>
                                <?php else: ?>
                                    <span style="color: #888;">Only the assigned staff/admin can change status.</span>
                                <?php endif; ?>
                            </p>
